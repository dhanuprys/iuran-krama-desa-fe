...
export default function AdminResidentDetailPage() {
  ...
  const handleApprove = async () => {
    ...
    try {
      await adminResidentService.validateResident(resident.id, { status: 'APPROVED' });
      toast.success('Penduduk berhasil disetujui.');
      fetchResident(resident.id);
    } catch (error) {
      console.error(error);
      toast.error('Gagal menyetujui penduduk.');
    } finally {
  ...
  const handleReject = async () => {
    if (!resident || !rejectionReason.trim()) return;
    setActionLoading(true);
    try {
      await adminResidentService.validateResident(resident.id, {
        status: 'REJECTED',
        rejection_reason: rejectionReason,
      });
      toast.success('Penduduk ditolak.');
      setRejectDialogOpen(false);
      setRejectionReason('');
      fetchResident(resident.id);
    } catch (error) {
      console.error(error);
      toast.error('Gagal menolak penduduk.');
    } finally {
      setActionLoading(false);
    }
  };
  ...
  if (error || !resident) {
    return (
      <LayoutContent>
        ...
        <Button variant="link" onClick={() => navigate('/admin/resident')} className="mt-4">
  ...
  return (
    <LayoutContent>
      ...
      <LayoutContentHeader
        backButton={<LayoutContentBackButton onClick={() => navigate('/admin/resident')} />}
        header={
          <div>
        ...
        info={
          <div className="flex gap-2">
            ...
            <Button onClick={() => navigate(`/admin/resident/${resident.id}/edit`)}>
                  ...
                  <div>
                    ...
                    {resident.validation_status === 'REJECTED' && resident.rejection_reason && (
                      <p className="text-destructive mt-1 text-sm">
                        Alasan: {resident.rejection_reason}
                      </p>
                    )}
                  </div>
                  <div className="grid grid-cols-2 gap-4">
      ...
      <AlertDialog open={rejectDialogOpen} onOpenChange={setRejectDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Tolak Pengajuan Penduduk</AlertDialogTitle>
            <AlertDialogDescription>
              Mohon berikan alasan penolakan untuk penduduk ini. Alasan ini akan tercatat.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <div className="py-2">
            <Textarea
